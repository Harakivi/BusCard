...
static __FORCEINLINE void render_bg_line(uint16_t* lineBuff, int y_axes)
{
    ...
    uint16_t *bufptr = &lineBuff[8 - dx_axes];
...
static __FORCEINLINE void render_sprite_pattern(uint16_t* lineBuff, SpriteType *sprptr, const uint8_t *Spr_Patterntable, uint16_t title_addr, uint8_t dy_axes)
{
    int i, dx_axes;
    ...
    if (sprptr->attr & SPR_HFLIP)
    {
        for (i = 7; i >= dx_axes; i--)
        {
            ...
            lineBuff[sprptr->x + i + 8] = NES_Color_Palette[PPU_Mem.sprite_palette[Spr_color_num]];
            //          if(Spr_color_num & 0x03 && (PPU_Reg.R1 & R1_SPR_LEFT8 || (sprptr -> x + 8 - i) > 8)){
            //              lineBuff[sprptr -> x + 8 - i] =  NES_Color_Palette[PPU_Mem.sprite_palette[Spr_color_num]];
            //          }
            //          else{
            //              lineBuff[sprptr -> x + 8 - i] =  NES_Color_Palette[PPU_Mem.sprite_palette[0]];
            //          }
        }
    }
    else
...
static __FORCEINLINE void render_sprite_88(uint16_t* lineBuff, SpriteType *sprptr, int dy_axes)
{
    const uint8_t *Spr_Patterntable;
    Spr_Patterntable = (PPU_Reg.R0 & SPR_PATTERN_ADDR) ? PPU_Mem.patterntable1 : PPU_Mem.patterntable0;
    render_sprite_pattern(lineBuff, sprptr, Spr_Patterntable, sprptr->t_num << 4, (uint8_t)dy_axes);
}

static __FORCEINLINE void render_sprite_16(uint16_t* lineBuff, SpriteType *sprptr, int dy_axes)
{
    if (sprptr->t_num & 0x01)
    {
        if (dy_axes < 8)
            render_sprite_pattern(lineBuff, sprptr, PPU_Mem.patterntable1, (sprptr->t_num & 0xFE) << 4, (uint8_t)dy_axes); // ��8*8
        else
            render_sprite_pattern(lineBuff, sprptr, PPU_Mem.patterntable1, sprptr->t_num << 4, (uint8_t)dy_axes & 7); // ��8*8
    }
    else
    {
        if (dy_axes < 8)
            render_sprite_pattern(lineBuff, sprptr, PPU_Mem.patterntable0, sprptr->t_num << 4, (uint8_t)dy_axes); // ��8*8
        else
            render_sprite_pattern(lineBuff, sprptr, PPU_Mem.patterntable0, (sprptr->t_num | 1) << 4, (uint8_t)dy_axes & 7); // ��8*8
    }
}


void ppu_render_line(uint16_t* lineBuff, int y_axes)
{
    ...
    if ((PPU_Reg.R1 & R1_BG_VISIBLE) && !(PPU_Reg.R1 & R1_SPR_VISIBLE))
    {
        for (i = 7; i < 256; i++)
        {
            lineBuff[i] = NES_Color_Palette[PPU_Mem.image_palette[0]];
        }
        render_bg_line(lineBuff, y_axes);
    }
    else if (PPU_Reg.R1 & (R1_BG_VISIBLE | R1_SPR_VISIBLE))
    {
        ...
        if (PPU_Reg.R1 & R1_SPR_VISIBLE)
        {
            ...
            for (i = 63; i >= 0; i--)
            {
                ...
                {
                    render_sprite_16(lineBuff, &sprite[i], dy_axes);
                }
                else
                {
                    render_sprite_88(lineBuff, &sprite[i], dy_axes);
                }
            }
        }
